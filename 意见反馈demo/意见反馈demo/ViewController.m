...
- (UIScrollView *)scrollView {
    ...
    if (!_scrollView) {
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(10, 64, 350, 200)];
//        _scrollView.b
    }
    return _scrollView;
}
...
//        } else { // preview photos / 预览照片
            ...
            imagePickerVc.maxImagesCount = kmax.integerValue;
...
- (void)pushTZImagePickerController {
    if (kmax.integerValue <= 0) {
        return;
    }
    NSString *str = @"4";
    TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:kmax.integerValue columnNumber:str.integerValue delegate:self pushPhotoPickerVc:YES];
    // imagePickerVc.navigationBar.translucent = NO;
    
#pragma mark - 五类个性化设置，这些参数都可以不传，此时会走默认设置
    imagePickerVc.isSelectOriginalPhoto = _isSelectOriginalPhoto;
    
    if (kmax.integerValue > 1) {
        // 1.设置目前已经选中的图片数组
...
- (UIButton *)btn{
    ...
    if (!_btn) {
        ...
        _btn.frame = CGRectMake(10, HeightVC-30, WidthVC-20, 30);
